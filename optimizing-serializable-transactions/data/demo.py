...
class Demo:
    ...
    def run(self):
        if self.demo == 1:
            return [self.select_normal, self.select_follower_read, self.select_normal_different_id, self.update_low]
        elif self.demo == 2:
            return [self.select_normal, self.select_follower_read, self.select_normal_different_id, self.update_high]
        elif self.demo == 3:
            return [self.select_normal, self.bulk_updates]
        else:
    ...
    def bulk_updates(self, conn: psycopg.Connection):
        with conn.transaction() as tx:
            with conn.cursor() as cur:
                cur.execute("SET TRANSACTION PRIORITY HIGH")
                cur.execute("""
